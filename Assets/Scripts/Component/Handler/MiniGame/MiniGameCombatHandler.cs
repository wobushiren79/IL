...
public class MiniGameCombatHandler : BaseMiniGameHandler<MiniGameCombatBuilder, MiniGameCombatBean>
{
    ...
    private IEnumerator CoroutineForCommandFight()
    {
        ...
        {
            //角色闪避了
            targetNpc.ShowTextInfo(TextHandler.Instance.GetTextById(14001),Color.blue);
        }
        //行动角色回到自己的位置
        yield return new WaitForSeconds(0.5f);
    ...
    private IEnumerator CoroutineForCommandSkill()
    {
        ...
        Sprite spSkill= IconHandler.Instance.GetIconSpriteByName(skillData.icon_key);
        //喊出技能名字
        actionNpc.SetShout(skillData.name_language);
