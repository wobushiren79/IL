...
public class UIPopupItemsShow : PopupShowView
{
    ...
    {
        string content = "???";
        if (data.GetItemsType() == GeneralEnum.Menu && data.content_language.IsNull())
        {
            MenuInfoBean menuInfo = InnFoodHandler.Instance.manager.GetFoodDataById(data.add_id);
            if (menuInfo != null)
                content = menuInfo.content_language;
        }
        else
        {
            content = data.content_language;
        }
        if (content.IsNull())
        {
    ...
    {
        string typeStr = TextHandler.Instance.GetTextById(400) + "：" + GeneralEnumTools.GetGeneralName(type);
        if (tvType != null)
            tvType.text = typeStr;
    }
    ...
    public void SetAttributes(ItemsInfoBean data)
    {
        ...
        CreateItemAttributes("ui_ability_cook", data.add_cook, TextHandler.Instance.GetTextById(1), colorForAttribute);
