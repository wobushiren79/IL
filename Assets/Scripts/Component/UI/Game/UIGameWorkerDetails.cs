...
public class UIGameWorkerDetails : BaseUIComponent, IRadioGroupCallBack
{
    ...
    public void InitDataByWorker(string name)
    {
        if (characterData == null)
            return;
        workerChefInfo.CloseUI();
        workerWaiterInfo.CloseUI();
        workerAccountantInfo.CloseUI();
        workerAccostInfo.CloseUI();
        workerBeaterInfo.CloseUI();
        workerSkillInfo.CloseUI();
        workerBookInfo.CloseUI();
        generalInfo.CloseUI();
    ...
    public void SetAttributes(CharacterBean characterData)
    {
        ...
        if (tvCook != null)
            tvCook.text = TextHandler.Instance.GetTextById(1) + "：" + selfAttributes.cook + (equipAttributes.cook == 0 ? "" : "+" + equipAttributes.cook);
        if (tvSpeed != null)
