...
public class NpcImportantBuilder : BaseMonoBehaviour
{
    ...
    private NpcAIImportantCpt BuildNpc(CharacterBean characterData)
    {
        ...
        try
        {
            ...
            {
                //检测是否满足出现条件
                if (!characterData.npcInfoData.condition.IsNull() && !ShowConditionTools.CheckIsMeetAllCondition(gameData, characterData.npcInfoData.condition))
                {
                    return null;
                }
            }
            ...
            if (talkBox != null)
            {
                if (characterData.npcInfoData.talk_types.IsNull())
                {
