...
public class ItemGameBackpackEquipCpt : ItemGameBackpackCpt
{
    ...
    public void SetSkillInfoData(List<SkillInfoBean> listData)
    {
        ...
        else
        {
            ...
            string toastStr = string.Format(TextHandler.Instance.GetTextById(1064), characterData.baseInfo.name, itemsInfoData.name_language);
    ...
    public override void SelectionUse(ItemsSelectionDialogView view)
    {
        ...
        switch (itemsType)
        {
            case GeneralEnum.Book:
                ...
                {
                    //已经学习过该图书
                    string toastStr = string.Format(TextHandler.Instance.GetTextById(1009), characterData.baseInfo.name, itemsInfoData.name_language);
            ...
            case GeneralEnum.SkillBook:
                ...
                {
                    //已经学习过该技能
                    string toastStr = string.Format(TextHandler.Instance.GetTextById(1063), characterData.baseInfo.name, itemsInfoData.name_language);
                    UIHandler.Instance.ToastHint<ToastView>(toastStr);
                }
                else
                {
            ...
            case GeneralEnum.Other:
                ...
                else if (itemsInfoData.id == 99900002)
                {
                    ...
                    {
                        //已经学习过该图书
                        string toastStr = string.Format(TextHandler.Instance.GetTextById(1055), characterData.baseInfo.name, itemsInfoData.name_language);
                        UIHandler.Instance.ToastHint<ToastView>(toastStr);
                    }
                    else
                    {
    ...
    protected void LearnBook()
    {
        ...
        string toastStr = string.Format(TextHandler.Instance.GetTextById(1008), characterData.baseInfo.name, itemsInfoData.name_language);
        UIHandler.Instance.ToastHint<ToastView>(ivIcon.sprite, toastStr);
        RefreshItems(itemsInfoData.id, -1);
    }
    ...
    protected void LearnItem()
    {
        ...
        characterData.attributes.AddAttributes(itemsInfoData);
        string toastStr = string.Format(TextHandler.Instance.GetTextById(1054), characterData.baseInfo.name, itemsInfoData.name_language);
        UIHandler.Instance.ToastHint<ToastView>(ivIcon.sprite, toastStr);
        RefreshItems(itemsInfoData.id, -1);
    }
}
