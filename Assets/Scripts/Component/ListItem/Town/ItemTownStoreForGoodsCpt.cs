...
public class ItemTownStoreForGoodsCpt : ItemTownStoreCpt
{
    ...
    public void SetData(StoreInfoBean storeInfo)
    {
        ...
        SetName(itemsInfo.name_language);
        SetContent(itemsInfo.content_language);
        SetOwn();
    ...
    public void SetAttribute(int add_book, int add_cook, int add_speed, int add_account, int add_charm, int add_force, int add_lucky)
    {
        ...
        if (tvCook != null)
            tvCook.text = TextHandler.Instance.GetTextById(1) + "+" + add_cook;
        if (tvSpeed != null)
        ...
        if (tvCookBook != null)
            tvCookBook.text = TextHandler.Instance.GetTextById(7) + "+" + add_book;
    }
    ...
    public override void SetOwn()
    {
        if(itemsInfo.GetItemsType()== GeneralEnum.Menu)
        {
            ...
            if (tvOwn != null)
                if (isHas)
                {
                    tvOwn.text = (TextHandler.Instance.GetTextById(195));
                }
                else
                {
                    tvOwn.text = (TextHandler.Instance.GetTextById(196));
    ...
    public virtual void Submit(DialogView dialogView, DialogBean dialogData)
    {
        ...
        if (dialogView as PickForNumberDialogView)
        {
            ...
            UIHandler.Instance.ToastHint<ToastView>(ivIcon.sprite, string.Format(TextHandler.Instance.GetTextById(1010), itemsInfo.name_language + "x" + (number * getNumber)));
            RefreshUI();
        }
    }
