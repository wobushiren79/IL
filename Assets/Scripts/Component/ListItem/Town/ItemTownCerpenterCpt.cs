...
public class ItemTownCerpenterCpt : ItemTownStoreCpt
{
    ...
    public void SetData(StoreInfoBean itemData)
    {
        ...
        if (type == StoreForCarpenterTypeEnum.Expansion)
        {
            ...
            name = storeInfo.name_language;
            content = storeInfo.content_language;
        }
        else
        {
            ...
            if (buildItemData != null)
            {
                ...
                name = buildItemData.name_language;
                content = buildItemData.content_language;
            }
        }
    ...
    public void SetAttribute(StoreForCarpenterTypeEnum type, float aesthetics)
    {
        ...
        if (tvAttribute != null)
            tvAttribute.text = TextHandler.Instance.GetTextById(10) + "+" + aesthetics;
    }
    ...
    public override void OnClickSubmitBuy()
    {
        ...
        {
            UIHandler.Instance.ToastHint<ToastView>(TextHandler.Instance.GetTextById(1019));
            return;
        }
        //检测金钱
        ...
        if (storeInfo.store_goods_type == (int)StoreForCarpenterTypeEnum.Expansion)
        {
            ...
            if (storeInfo.store_goods_type == (int)StoreForCarpenterTypeEnum.Expansion)
            {
                ...
                {
                    dialogBean.content = string.Format(TextHandler.Instance.GetTextById(3016));
                }
                else
                {
                    dialogBean.content = string.Format(TextHandler.Instance.GetTextById(3010), 1 + "");
                }
            }
            else
                dialogBean.content = string.Format(TextHandler.Instance.GetTextById(3002), buildItemData.name_language);
    ...
    public void Submit(DialogView dialogView, DialogBean dialogData)
    {
        ...
        if (dialogView as PickForNumberDialogView)
        {
            ...
            {
                UIHandler.Instance.ToastHint<ToastView>(TextHandler.Instance.GetTextById(1012));
                return;
            }
            ...
            {
                UIHandler.Instance.ToastHint<ToastView>(TextHandler.Instance.GetTextById(1021));
                return;
            }
            ...
            RefreshUI();
            string toastStr = string.Format(TextHandler.Instance.GetTextById(1010), buildItemData.name_language + "x" + (number * getNumber));
        ...
        else
        {
            ...
            if (storeInfo.store_goods_type == (int)StoreForCarpenterTypeEnum.Expansion)
            {
                ...
                toastStr = string.Format(TextHandler.Instance.GetTextById(1011), storeInfo.name_language);
            }
            else
            {
                //加上获取数量
                ...
                RefreshUI();
                toastStr = string.Format(TextHandler.Instance.GetTextById(1010), buildItemData.name_language + "x" + 1 * getNumber);
            }
            UIHandler.Instance.ToastHint<ToastView>(ivIcon.sprite, toastStr);
        }
    }
