...
public class MiniGameCombatBuilder : BaseMiniGameBuilder
{
    ...
    /// <returns></returns>
    public void CreateCombatEffect(string combatEffectName, Vector3 position, Action<EffectBase> callBackForComplete = null)
    {
        EffectBean effectData = new EffectBean();
        effectData.effectName = combatEffectName;
        effectData.effectPosition = position;
        EffectHandler.Instance.ShowEffect(effectData, (effect) =>
        {
            callBackForComplete?.Invoke(effect);
