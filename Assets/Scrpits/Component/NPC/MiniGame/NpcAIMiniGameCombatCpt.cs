...
public class NpcAIMiniGameCombatCpt : BaseNpcAI
{
    ...
    {
        List<BufferTypeBean> listTypeData = BufferTypeEnumTools.GetListEffectData(effectTypeData);
        ...
        foreach (BufferTypeBean itemType in listTypeData)
        {
            //设置技能的备用图标
            BufferTypeEnumTools.GetEffectDetails(itemType, spIcon);
            //如果是持续 则需要加上BUFF图标
